...
System.register("src/geometry", [], function (exports_2, context_2) {
    "use strict";
    ...
    var EPSILON_LOW, EPSILON, EPSILON_HIGH, EPSILON_UI, Tree, Matrix, XY, LineType, Line, Ray, Edge, Polyline, Rect, Triangle, IsoscelesTriangle, Circle, ConvexPolygon, Sector, VoronoiMolecule, VoronoiMoleculeTriangle, VoronoiEdge, VoronoiCell, VoronoiJunction, VoronoiGraph, flatMap;
    return {
        setters: [],
        execute: function () {
            EPSILON_LOW = 0.003;
            EPSILON = 0.00001;
            EPSILON_HIGH = 0.00000001;
            EPSILON_UI = 0.05;
            Tree = class Tree {
            ...
            XY = class XY {
                ...
                commonX(point, epsilon) { return epsilonEqual(this.y, point.y, epsilon); }
                commonY(point, epsilon) { return epsilonEqual(this.x, point.x, epsilon); }
            };
            exports_2("XY", XY);
            LineType = class LineType {
                length() { }
                vector() { }
            ...
            exports_2("Triangle", Triangle);
            IsoscelesTriangle = class IsoscelesTriangle extends Triangle {
            };
            Circle = class Circle {
                constructor(a, b, c) {
                    if (c !== undefined) {
            ...
            ConvexPolygon = class ConvexPolygon {
                center() {
            ...
            exports_2("ConvexPolygon", ConvexPolygon);
            Sector = class Sector {
            ...
            exports_2("Sector", Sector);
            VoronoiMolecule = class VoronoiMolecule extends Triangle {
            ...
            VoronoiMoleculeTriangle = class VoronoiMoleculeTriangle {
            ...
            VoronoiEdge = class VoronoiEdge {
                constructor() {
                    this.cache = {};
                }
            };
            VoronoiCell = class VoronoiCell {
                constructor() { this.points = []; this.edges = []; }
            };
            VoronoiJunction = class VoronoiJunction {
                constructor() { this.edges = []; this.cells = []; this.isEdge = false; this.isCorner = false; }
            };
            VoronoiGraph = class VoronoiGraph {
            ...
            exports_2("VoronoiGraph", VoronoiGraph);
            flatMap = function (array, mapFunc) {
                return array.reduce((cumulus, next) => [...mapFunc(next), ...cumulus], []);
            };
        }
    };
});
System.register("src/planarGraph", ["src/geometry", "src/graph"], function (exports_3, context_3) {
    "use strict";
    var __moduleName = context_3 && context_3.id;
    var M, graph_1, PlanarClean, PlanarNode, PlanarEdge, PlanarFace, PlanarSector, PlanarJunction, PlanarGraph;
    return {
        setters: [
            function (M_1) {
                M = M_1;
        ...
        execute: function () {
            ...
            exports_3("PlanarClean", PlanarClean);
            PlanarNode = class PlanarNode extends graph_1.GraphNode {
                ...
                midpoint(other) { return new M.XY((this.x + other.x) * 0.5, (this.y + other.y) * 0.5); }
                abs() { this.x = Math.abs(this.x), this.y = Math.abs(this.y); return this; }
                commonX(p, epsilon) { return new M.XY(this.x, this.y).commonX(new M.XY(p.x, p.y), epsilon); }
                commonY(p, epsilon) { return new M.XY(this.x, this.y).commonY(new M.XY(p.x, p.y), epsilon); }
            };
            exports_3("PlanarNode", PlanarNode);
            PlanarEdge = class PlanarEdge extends graph_1.GraphEdge {
                ...
                midpoint() {
                    return new M.XY(0.5 * (this.nodes[0].x + this.nodes[1].x), 0.5 * (this.nodes[0].y + this.nodes[1].y));
                }
                perpendicularBisector() { return new M.Line(this.midpoint(), this.vector().rotate90()); }
                infiniteLine() {
            ...
            exports_3("PlanarEdge", PlanarEdge);
            ...
            exports_3("PlanarFace", PlanarFace);
            PlanarSector = class PlanarSector extends M.Sector {
            ...
            exports_3("PlanarSector", PlanarSector);
            ...
            exports_3("PlanarJunction", PlanarJunction);
            ...
            exports_3("PlanarGraph", PlanarGraph);
        }
    };
});
System.register("src/creasePattern", ["src/geometry", "src/planarGraph"], function (exports_4, context_4) {
    "use strict";
    ...
    function gimme1XY(a, b) {
        if (isValidPoint(a)) {
            return new M.XY(a.x, a.y);
        }
        if (isValidNumber(b)) {
            return new M.XY(a, b);
        }
        if (a.constructor === Array) {
            return new M.XY(a[0], a[1]);
        }
    }
    function gimme2XY(a, b, c, d) {
        if (a instanceof M.XY && b instanceof M.XY) {
            return [a, b];
        }
        if (isValidPoint(b)) {
            return [new M.XY(a.x, a.y), new M.XY(b.x, b.y)];
        }
        if (isValidNumber(d)) {
            return [new M.XY(a, b), new M.XY(c, d)];
        }
    }
    function gimme1Edge(a, b, c, d) {
        if (a instanceof M.Edge) {
            return a;
        }
        if (a.nodes !== undefined) {
            return new M.Edge(a.nodes[0], a.nodes[1]);
        }
        if (isValidPoint(b)) {
            return new M.Edge(a, b);
        }
        if (isValidNumber(d)) {
            return new M.Edge(a, b, c, d);
        }
    }
    function gimme1Ray(a, b, c, d) {
        ...
        if (isValidPoint(b)) {
            return new M.Ray(a, b);
        }
        if (isValidNumber(d)) {
            return new M.Ray(new M.XY(a, b), new M.XY(c, d));
        }
    }
    function gimme1Line(a, b, c, d) {
        ...
        if (isValidPoint(b)) {
            return new M.Line(a, b);
        }
        if (isValidNumber(d)) {
            return new M.Line(a, b, c, d);
        }
    ...
    var M, planarGraph_1, CreaseDirection, Fold, MadeByType, MadeBy, ChangeType, FoldSequence, CPPoint, CPVector, CreaseSector, CreaseJunction, CreaseNode, Crease, CreaseFace, CreasePattern;
    return {
        setters: [
            function (M_2) {
                M = M_2;
        ...
        execute: function () {
            ...
            CPPoint = class CPPoint extends M.XY {
            };
            exports_4("CPPoint", CPPoint);
            CPVector = class CPVector extends M.XY {
            };
            exports_4("CPVector", CPVector);
            ...
            CreasePattern = class CreasePattern extends planarGraph_1.PlanarGraph {
                ...
                creaseThroughLayers(point, vector) {
                ...
                creasePerpendicularThroughPoint(line, point) {
                    var edge = this.boundary.clipLine(new M.Line(point, line.vector().rotate90()));
                    if (edge === undefined) {
                        return;
                    }
                    var newCrease = this.newCrease(edge.nodes[0].x, edge.nodes[0].y, edge.nodes[1].x, edge.nodes[1].y);
                ...
                creasePointToLine(origin, point, line) {
                    ...
                    var intersections = new M.Circle(origin.x, origin.y, radius).intersection(new M.Edge(line));
                    var creases = [];
                    for (var i = 0; i < intersections.length; i++) {
                ...
                creasePerpendicularPointOntoLine(point, ontoLine, perp) {
                    var newLine = new M.Line(point, new M.XY(perp.nodes[1].x - perp.nodes[0].x, perp.nodes[1].y - perp.nodes[0].y));
                    var intersection = newLine.intersection(ontoLine.infiniteLine());
                    if (intersection === undefined) {
                        return;
                    }
                    var edge = this.boundary.clipLine(new M.Edge(point, intersection).perpendicularBisector());
                    if (edge === undefined) {
                        return;
                    }
                    return this.newCrease(edge.nodes[0].x, edge.nodes[0].y, edge.nodes[1].x, edge.nodes[1].y);
                ...
                pleat(one, two, count) {
                    ...
                    if (a.parallel(b)) {
                        ...
                        return Array.apply(null, Array(count - 1))
                            ...
                            return this.boundary.clipLine(new M.Line(origin, u));
                        }, this)
                        ...
                        return Array.apply(null, Array(count - 1))
                            ...
                            .map(function (el) {
                            var vector = u.angleLerp(v, el);
                            return this.boundary.clipLine(new M.Line(intersection, vector));
                        }, this)
                            .filter(function (el) { return el !== undefined; }, this)
                            .map(function (el) { return this.newCrease(el.nodes[0].x, el.nodes[0].y, el.nodes[1].x, el.nodes[1].y); }, this);
                    }
                    ...
                    for (var i = 0; i < this.nodes.length; i++) {
                        ...
                        if (rating > epsilon) {
                            ...
                            for (var n = 0; n < 12; n++) {
                                ...
                                var move = new M.XY(0.05 * radius * Math.cos(randomAngle), 0.05 * radius * Math.sin(randomAngle));
                ...
                setBoundary(pointArray, alreadyClockwiseSorted) {
                ...
                setMinRectBoundary() {
                    ...
                    this.setBoundary([new M.XY(xMin, yMin), new M.XY(xMax, yMin), new M.XY(xMax, yMax), new M.XY(xMin, yMax)]);
                    return this;
                }
                clear() {
                    this.nodes = [];
