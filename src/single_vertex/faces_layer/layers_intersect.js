...
const layers_intersect = (folded_faces, assignments, layers_face, epsilon = math.core.EPSILON) => {
  // console.log("layers intersect", folded_faces, assignments, layers_face);
  ...
  // console.log("fold_location", fold_location);
  ...
  const faces_mins = folded_faces
    ...
    .map(n => n + epsilon);
  // console.log("faces_mins", faces_mins);
  // for every sector, the location of the end which lies nearest to +Infinity
  const faces_maxs = folded_faces
    .map(ends => ends ? Math.max(...ends) : undefined)
    .map(n => n - epsilon);
  // console.log("faces_maxs", faces_maxs);
