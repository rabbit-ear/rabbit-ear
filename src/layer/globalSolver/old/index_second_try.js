...
const filterTransitivity = (overlapping_face_trios, tacos_tortillas) => {
  const tacos_trios = {};
...
const makeConditions = (graph, epsilon = 1e-6) => {
  const start_conditions = {};

  const overlap_matrix = makeFacesFacesOverlap(graph, epsilon);
  booleanMatrixToUniqueIndexPairs(half_matrix(overlap_matrix))
    .map(pair => pair.join(" "))
  ...
  const unfiltered_trios = makeTransitivityTrios(graph, overlap_matrix, epsilon);
