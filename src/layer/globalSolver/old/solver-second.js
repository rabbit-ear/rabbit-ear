...
const completeSuggestionsLoop = (conditions, maps, layers) => {
...
const solver = (graph, maps, overlap_matrix, faces_winding) => {
		...
		const next_steps = zero_keys
			.map(key => [-1, 1]
				.map(dir => {
					...
					const success = completeSuggestionsLoop(clone_conditions, maps, clone_layers);
	...
	if (!completeSuggestionsLoop(conditions_start, maps, layers_start)) {
