...
export const intersectConvexPolygonLine = (
	...
	if (uniqueIncludes === undefined) {
		...
		case excludeR:
			...
			return overlapConvexPolygonPoint(poly, origin, exclude, 1e-3).overlap
				? includes
				: undefined;
		...
		case excludeS:
			return overlapConvexPolygonPoint(poly, add2(origin, vector), exclude, 1e-3).overlap
				|| overlapConvexPolygonPoint(poly, origin, exclude, 1e-3).overlap
				? includes
				: undefined;
	...
	return overlapConvexPolygonPoint(poly, midpoint2(...uniqueIncludes), exclude, 1e-3).overlap
		? uniqueIncludes
		: sects;
};
