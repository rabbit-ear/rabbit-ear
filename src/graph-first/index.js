...
const Graph = function () {
   ...
   * @param {Node} node1 the first node that the edge connects
   * @param {Node} node2 the second node that the edge connects
   * @returns {Edge} if successful, pointer to the edge
   * @example
  ...
  *     and adds it to the graph
   ...
   *    and adds it to the graph
   * @returns {Edge} pointer to the edge
   */
   ...
   * @param {Node} node1 first node to merge, this node will persist
   * @param {Node} node2 second node to merge, this node will be removed
   ...
   * @returns {Edge} edge if exists. undefined if nodes are not adjacent
   * @example
   ...
   * duplicate edges.
   * @returns {Edge[]} array of edges, if any exist. empty array if
