...
vecText = RabbitEar.svg("canvas-vector-labels", window.innerWidth, window.innerHeight, function () {
  if (vecText != null) {
...
vecText.redraw = function () {
  ...
  let crossEnd = Math.atan2(vecText.rot90.y, vecText.rot90.x);
  let crossStart = Math.atan2(vecText.v.y, vecText.v.x);
  let crossArc1 = RabbitEar.svg.arc(0, 0, crossLen, crossStart, crossEnd);
