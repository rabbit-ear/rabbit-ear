...
for (var i = 0; i < params.length; i++) {
	// let point = re.vector(Math.random(), Math.random());
...
// let point = re.vector(0.2, 0.8);
// let vector = re.vector(1, 1);
// let stay = re.vector(1, -1);
// valleys.cp.valleyFold(point, vector, stay);

valleys.masterCP = valleys.cp.copy();
...
valleys.update = function() {
	...
	if(yes){
		...
		let points = [
			re.vector(valleys.controls[0].position),
			re.vector(valleys.controls[1].position)
		];
	...
	folded.cp = valleys.cp.copy();
	let notMoving = folded.cp["faces_re:to_move"].indexOf(false);
