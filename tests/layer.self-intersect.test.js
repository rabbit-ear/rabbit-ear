...
test("self_intersect", () => {
	...
	const res1 = ear.layer.validate_taco_tortilla_strip(folded_faces, layers_face, false, 0.001);
	expect(res1).toBe(true);
	const res2 = ear.layer.validate_taco_tortilla_strip(folded_faces, layers_face, true, 0.001);
	...
	for (let i = 0; i < folded_faces.length; i++) {
		...
		expect(ear.layer.validate_taco_tortilla_strip(folded_faces, layers_face, true, 0.001))
			.toBe(true);
	}
});
...
test("self-intersect, taco tortilla, valid (not loop)", () => {
	...
	const res = ear.layer.validate_taco_tortilla_strip(folded_faces, layers_face, false, 0.001);
