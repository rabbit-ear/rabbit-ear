...
test("collinear line", () => {
  ...
  const result1 = ear.math.clipLineConvexPolygon(rect, ...lineHoriz1, include, includeL);
  const result2 = ear.math.clipLineConvexPolygon(rect, ...lineHoriz2, include, includeL);
  const result3 = ear.math.clipLineConvexPolygon(rect, ...lineVert1, include, includeL);
  const result4 = ear.math.clipLineConvexPolygon(rect, ...lineVert2, include, includeL);
  const result5 = ear.math.clipLineConvexPolygon(rect, ...lineHoriz1, exclude, excludeL);
  const result6 = ear.math.clipLineConvexPolygon(rect, ...lineHoriz2, exclude, excludeL);
  const result7 = ear.math.clipLineConvexPolygon(rect, ...lineVert1, exclude, excludeL);
  const result8 = ear.math.clipLineConvexPolygon(rect, ...lineVert2, exclude, excludeL);
...
test("vertex-incident line", () => {
  ...
  const results = [
    ear.math.clipLineConvexPolygon(quad, ...lineHoriz1, include, includeL),
    ear.math.clipLineConvexPolygon(quad, ...lineHoriz2, include, includeL),
    ear.math.clipLineConvexPolygon(quad, ...lineVert1, include, includeL),
    ear.math.clipLineConvexPolygon(quad, ...lineVert2, include, includeL),
    ear.math.clipLineConvexPolygon(quad, ...lineHoriz1, exclude, excludeL),
    ear.math.clipLineConvexPolygon(quad, ...lineHoriz2, exclude, excludeL),
    ear.math.clipLineConvexPolygon(quad, ...lineVert1, exclude, excludeL),
    ear.math.clipLineConvexPolygon(quad, ...lineVert2, exclude, excludeL),
  ];
...
test("collinear core, segment", () => {
  ...
  const result1 = ear.math.clipLineConvexPolygon(rect, ...segHoriz1, include, includeS);
  const result2 = ear.math.clipLineConvexPolygon(rect, ...segHoriz2, include, includeS);
  const result3 = ear.math.clipLineConvexPolygon(rect, ...segVert1, include, includeS);
  const result4 = ear.math.clipLineConvexPolygon(rect, ...segVert2, include, includeS);
  const result5 = ear.math.clipLineConvexPolygon(rect, ...segHoriz1, exclude, excludeS);
  const result6 = ear.math.clipLineConvexPolygon(rect, ...segHoriz2, exclude, excludeS);
  const result7 = ear.math.clipLineConvexPolygon(rect, ...segVert1, exclude, excludeS);
  const result8 = ear.math.clipLineConvexPolygon(rect, ...segVert2, exclude, excludeS);
  expect(result1.length).toBe(2);
  expect(result2.length).toBe(2);
  expect(result3.length).toBe(2);
  expect(result4.length).toBe(2);
  ...
  const result9 = ear.math.clipLineConvexPolygon(rect, ...segHoriz3, include, includeS);
  const result10 = ear.math.clipLineConvexPolygon(rect, ...segVert3, include, includeS);
  const result11 = ear.math.clipLineConvexPolygon(rect, ...segHoriz3, exclude, excludeS);
  const result12 = ear.math.clipLineConvexPolygon(rect, ...segVert3, exclude, excludeS);
...
test("vertex-incident segment", () => {
  ...
  const vert2 = [[0, 1], [1, -1]];
  const results = [
    ear.math.clipLineConvexPolygon(quad, ...horiz1, include, includeS),
    ear.math.clipLineConvexPolygon(quad, ...horiz2, include, includeS),
    ear.math.clipLineConvexPolygon(quad, ...vert1, include, includeS),
    ear.math.clipLineConvexPolygon(quad, ...vert2, include, includeS),
    ear.math.clipLineConvexPolygon(quad, ...horiz1, exclude, excludeS),
    ear.math.clipLineConvexPolygon(quad, ...horiz2, exclude, excludeS),
    ear.math.clipLineConvexPolygon(quad, ...vert1, exclude, excludeS),
    ear.math.clipLineConvexPolygon(quad, ...vert2, exclude, excludeS),
  ];
  results.forEach(res => expect(res).toBe(undefined));
});
...
test("collinear core, ray", () => {
  ...
  const result1 = ear.math.clipLineConvexPolygon(rect, ...rayHoriz1, include, includeR);
  const result2 = ear.math.clipLineConvexPolygon(rect, ...rayHoriz2, include, includeR);
  const result3 = ear.math.clipLineConvexPolygon(rect, ...rayVert1, include, includeR);
  const result4 = ear.math.clipLineConvexPolygon(rect, ...rayVert2, include, includeR);
  const result5 = ear.math.clipLineConvexPolygon(rect, ...rayHoriz1, exclude, excludeR);
  const result6 = ear.math.clipLineConvexPolygon(rect, ...rayHoriz2, exclude, excludeR);
  const result7 = ear.math.clipLineConvexPolygon(rect, ...rayVert1, exclude, excludeR);
  const result8 = ear.math.clipLineConvexPolygon(rect, ...rayVert2, exclude, excludeR);
  expect(result1.length).toBe(2);
  expect(result2.length).toBe(2);
  expect(result3.length).toBe(2);
  expect(result4.length).toBe(2);
...
test("vertex-incident ray", () => {
  ...
  const vert2 = [[0, 1], [1, -1]];
  const results = [
    ear.math.clipLineConvexPolygon(quad, ...horiz1, include, includeR),
    ear.math.clipLineConvexPolygon(quad, ...horiz2, include, includeR),
    ear.math.clipLineConvexPolygon(quad, ...vert1, include, includeR),
    ear.math.clipLineConvexPolygon(quad, ...vert2, include, includeR),
    ear.math.clipLineConvexPolygon(quad, ...horiz1, exclude, excludeR),
    ear.math.clipLineConvexPolygon(quad, ...horiz2, exclude, excludeR),
    ear.math.clipLineConvexPolygon(quad, ...vert1, exclude, excludeR),
    ear.math.clipLineConvexPolygon(quad, ...vert2, exclude, excludeR),
  ];
  results.forEach(res => expect(res).toBe(undefined));
});
