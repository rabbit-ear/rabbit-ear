...
test("alternating sum", () => {
	testArrays([0, 0], ear.single.alternating_sum([]));
	testArrays([1, 0], ear.single.alternating_sum([1]));
	testArrays([16, 20], ear.single.alternating_sum([1, 2, 3, 4, 5, 6, 7, 8]));
});

test("alternating deviation, equal pairs", () => {
	const equal2 = [Math.PI, Math.PI];
	const equal4 = Array(4).fill(Math.PI / 2);
	testArrays([0, 0], ear.single.alternating_sum_difference(equal2));
	testArrays([0, 0], ear.single.alternating_sum_difference(equal4));
	testArrays([0, 0],
	  ear.single.alternating_sum_difference(
...
test("alternating deviation, non-equal pairs", () => {
  ...
  // four Math.PI/2 +/- 1/2
	...
	testArrays([-1, 1], ear.single.alternating_sum_difference(arr2));
	testArrays([-1, 1], ear.single.alternating_sum_difference(arr4));
});

test("kawasaki solutions radians", () => {
	testArrays(
	  [undefined, undefined, 1.25 * Math.PI],
	  ear.single.kawasaki_solutions_radians([
