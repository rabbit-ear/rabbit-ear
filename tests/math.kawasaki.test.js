...
test("alternating sum", () => {
	testArrays([0, 0], ear.math.alternating_sum([]));
	testArrays([1, 0], ear.math.alternating_sum([1]));
	testArrays([16, 20], ear.math.alternating_sum([1, 2, 3, 4, 5, 6, 7, 8]));
});

test("alternating deviation, equal pairs", () => {
	const equal2 = [Math.PI, Math.PI];
	const equal4 = Array(4).fill(Math.PI / 2);
	testArrays([0, 0], ear.math.alternating_deviation(equal2));
	testArrays([0, 0], ear.math.alternating_deviation(equal4));
	testArrays([0, 0],
	  ear.math.alternating_deviation(
		  ear.math.interior_angles([1, 0], [0, 1], [-1, 0], [0, -1])));
});
...
test("alternating deviation, non-equal pairs", () => {
  ...
  // four Math.PI/2 +/- 1/2
	...
	testArrays([-1, 1], ear.math.alternating_deviation(arr2));
	testArrays([-1, 1], ear.math.alternating_deviation(arr4));
});
