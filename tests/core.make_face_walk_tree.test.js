...
test("face walk tree, crane", () => {
  ...
  const tree = ear.core.make_face_walk_tree(crane, startingFace);
...
test("face walk tree, empty graph", () => {
  const result = ear.core.make_face_walk_tree({
    edges_vertices: [],
    faces_vertices: [],
  });
...
test("face walk tree, finding an edge match. bad edge formations", () => {
  // good edge
  const result0 = ear.core.make_face_walk_tree({
    edges_vertices: [[1, 3]],
    faces_vertices: [[0, 1, 3], [2, 3, 1]],
  });
