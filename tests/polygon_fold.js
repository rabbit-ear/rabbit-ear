...
polyFold.reset = function(){
	...
	polyFold.touches = Array.from(Array(2)).map(_ => ({
		pos: [Math.random()*polyFold.width, Math.random()*polyFold.height],
		svg: RabbitEar.svg.circle(0, 0, polyFold.RADIUS)
	}));
	polyFold.touches[0].pos = [polyFold.width*0.5, polyFold.height*0.5];
	...
	let hullPoints = Array.from(Array(24)).map(_ => {
		...
		let r = Math.random() * polyFold.height*0.5;
		return [polyFold.width*0.5 + r*Math.cos(a), polyFold.height*0.5 + r*Math.sin(a)];
	});
