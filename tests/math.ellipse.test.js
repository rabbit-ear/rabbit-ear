...
// test("nearestPoint", () => {
//   const result = ear.ellipse(2, 1).nearestPoint(0, 0);
// };
// test("intersect", () => {
//   const result = ear.ellipse(2, 1).intersect(object);
// };
test("svgPath", () => {
	expect(ear.ellipse(2, 1).svgPath()).toBe("M2 0A2 1 0 0 1 -2 0A2 1 0 0 1 2 0");
	expect(ear.ellipse(2, 1, 5, 6).svgPath()).toBe("M7 6A2 1 0 0 1 3 6A2 1 0 0 1 7 6");
});
test("points", () => {
	expect(ear.ellipse(2, 1).points().length).toBe(128);
	expect(ear.ellipse(2, 1).points(10).length).toBe(10);
});
test("polygon", () => {
	const result = ear.ellipse(2, 1).polygon();
	expect(result.length).toBe(128);
	expect(result[0][0]).toBeCloseTo(2);
	expect(result[0][1]).toBeCloseTo(0);
});
test("segments", () => {
	const result = ear.ellipse(2, 1).segments();
	expect(result.length).toBe(128);
