...
test("make faces_faces, square", () => {
  const result = ear.graph.make_faces_faces({
...
test("make faces_faces, invalid faces", () => {
  const res1 = ear.graph.make_faces_faces({
  ...
  const res2 = ear.graph.make_faces_faces({ faces_vertices: [["hi"], ["hi"]] });
  expect(res2[0]).toEqual(expect.arrayContaining([1]));
  expect(res2[1]).toEqual(expect.arrayContaining([0]));
  const res3 = ear.graph.make_faces_faces({ faces_vertices: [["hi"], ["bye"]] });
...
test("make faces_faces, degenerate", () => {
  ...
  const res0 = ear.graph.make_faces_faces({
    faces_vertices: [[0, 0]]
  });
