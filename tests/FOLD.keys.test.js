...
test("edge angle assinments", () => {
  ...
  const calculated = assignments.map(a => re.core.edge_assignment_to_foldAngle(a));
  expect(re.equivalent(calculated, angles)).toBe(true);
});
...
test("transpose geometry arrays", () => {
  ...
  expect(expectedVertex1.coords).toEqual(
    expect.arrayContaining(transposedVertices[1].coords)
  );
  expect(expectedVertex1.faces).toEqual(
    expect.arrayContaining(transposedVertices[1].faces)
  );
  expect(expectedVertex1.vertices).toEqual(
    expect.arrayContaining(transposedVertices[1].vertices)
  );
});


test("get keys with ending", () => {
  // re.core.get_keys_with_ending();
