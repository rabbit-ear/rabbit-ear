...
test("layer solver, wrong final crease direction3, invalid edge-crossing", () => {
	...
	//           (----2---)
	//	
	const invalid = ear.layer.single_vertex_solver([4, 2, 2, 4], Array.from("MMVV"));
	expect(invalid.length).toBe(0);
	expect(JSON.stringify(invalid)).toBe(JSON.stringify([]));
});
...
test("layer solver, simple staircase, wraparound, final crease wrong direction 2", () => {
	const res = ear.layer.single_vertex_solver([3,2,3,2,3,5], Array.from("MVMVMV"));
	expect(res.length).toBe(0);
	expect(JSON.stringify(res)).toBe(JSON.stringify([]));
});

// test("layer solver, incident touching, valid", () => {
// 	const res = ear.layer.single_vertex_solver([6,3,3,6,3,3], Array.from("VVMVVM"));
...
test("layer solver, simple fold", () => {
	const res1 = ear.layer.single_vertex_solver([2,2,2,2], Array.from("VVVM"));
	const res2 = ear.layer.single_vertex_solver([2,2,2,2], Array.from("VVMV"));
	const res3 = ear.layer.single_vertex_solver([2,2,2,2], Array.from("VMVV"));
	const res4 = ear.layer.single_vertex_solver([2,2,2,2], Array.from("MVVV"));
...
test("layer_solver, loop around", () => {
	...
	const res = ear.layer.single_vertex_solver(faces, assignments);
	expect(res.length).toBe(1);

	expect(ear.layer.single_vertex_solver([2, 4, 4, 2], ["V","M","V","V"]).length)
		.toBe(1);

	expect(ear.layer.single_vertex_solver([4, 4, 2, 2], ["V","V","M","V"]).length)
		.toBe(1);

	expect(ear.layer.single_vertex_solver([4, 2, 2, 4], ["V","V","V","M"]).length)
		.toBe(1);
});
...
test("layer_solver. from crane", () => {
	...
	const assignments = ["M","V","V","V"];
	const res = ear.layer.single_vertex_solver(faces, assignments, 0.001);
