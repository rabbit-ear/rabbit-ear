var _03_add_nodes_snake = function( p ) {
	...
	p.draw = function() {
		...
		if(mouseDownLocation != undefined){
			...
			p.line(g.nodes[g.nodes.length-1].x, g.nodes[g.nodes.length-1].y, mouseXScaled, mouseYScaled);
			if(p.callback != undefined){
				p.callback({ 'start':{'x':g.nodes[g.nodes.length-1].x, 'y':g.nodes[g.nodes.length-1].y}, 
