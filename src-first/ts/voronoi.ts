...
// import * as d3Voronoi from './../lib/d3-voronoi'
...
interface d3VoronoiAlgorithm{
	(data:any): any;
	polygons(data:any): any;
	links(data:any): any;
	triangles(data:any): any;
	x(_: any, ...args: any[]): (d:any) => any;
	y(_: any, ...args: any[]): (d:any) => any;
	extent(_: any, ...args: any[]): any | any[][];
	size(_: any, ...args: any[]): any | number[];
}

interface d3{
	voronoi():d3VoronoiAlgorithm;
} declare var d3;
...
export class VoronoiGraph{
	...
	constructor(points:any, bounds?:M.Rect, epsilon?:number){
		// points
		// var boundsArray = [bounds.origin.x, bounds.origin.y, bounds.size.width, bounds.size.height];
		var boundsArray = [[bounds.origin.x, bounds.origin.y], [bounds.size.width, bounds.size.height]];
		var voronoiAlgorithm:d3VoronoiAlgorithm = <d3VoronoiAlgorithm>(d3.voronoi().extent( boundsArray ));
		var v:d3VoronoiObject = <d3VoronoiObject>voronoiAlgorithm( points );
