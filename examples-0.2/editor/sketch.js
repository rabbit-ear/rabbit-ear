...
origami.updatePenPath = function () {
  ...
  let m0 = RE.Vector(origami.mouse[0], origami.mouse[1]);
  let m1 = RE.Vector(origami.mouse.pressed[0], origami.mouse.pressed[1]);
  let perp = m0.subtract(m1).normalize().scale(0.3).rotateZ90();
  let perp1 = m0.midpoint(m1).add(perp);
  let perp2 = m0.midpoint(m1).add(perp.scale(-1));
