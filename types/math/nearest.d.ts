...
export function nearestPointOnLine({ vector, origin }: VecLine, point: number[], clampFunc?: Function, epsilon?: number): number[];
export function nearestPointOnPolygon(polygon: [number, number][], point: [number, number]): object;
export function nearestPointOnCircle({ radius, origin }: Circle, point: number[]): number[];
