...
function split_convex_polygon(poly, linePoint, lineVector){
	...
	let vertices_intersections = poly
		.map(v => point_on_line(linePoint, lineVector, v))
	...
	if(edges_intersections.length == 2){
		let sorted_indices = edges_intersections.slice()
			.sort((a,b) => a.at_index - b.at_index);

		let face_a = poly.slice(sorted_indices[1].at_index+1);
		face_a = face_a.concat(poly.slice(0, sorted_indices[0].at_index+1));
		face_a.push(sorted_indices[0].point);
		face_a.push(sorted_indices[1].point);
		// face b
		let face_b = poly
			.slice(sorted_indices[0].at_index+1, sorted_indices[1].at_index+1);
		face_b.push(sorted_indices[1].point);
		face_b.push(sorted_indices[0].point);
		return [face_a, face_b];
	} else if(edges_intersections.length == 1 && vertices_intersections == 1);
}
...
function Polygon(){
	...
	let _points = get_array_of_vec2(...arguments);
...
export { intersection, core, input, Matrix, Vector, Line, Ray, Edge$1 as Edge, Circle, Polygon };
