...
//		this.cpLayer.removeChildren();
		...
		for(var i = 0; i < this.cp.edges.length; i++){
			var path;
			if(     cp.edges[i].orientation == CreaseDirection.mountain){ path = mountainPath(this.lineWeight); }
			else if(cp.edges[i].orientation == CreaseDirection.valley){ path = valleyPath(this.lineWeight); }
