var cp = new CreasePattern().rectangle(1, 1.618);
...
origami.bounds = cp.bounds();
for(var i = 0; i < 3; i++){
	var dir = (i%2 * 2)-1;
	origami.makeTouchPoint(new XY(origami.bounds.size.width*0.5, origami.bounds.size.height*0.5 + dir*(i+0.5)*0.25 ));
	origami.makeTouchPoint(new XY(origami.bounds.size.width*0.5, origami.bounds.size.height*0.5 - dir*(i+0.5)*0.25 ));
}

origami.makeTouchPoint(new XY(0, origami.bounds.size.height*0.5 - 0.015 ));
origami.makeTouchPoint(new XY(0, origami.bounds.size.height*0.5 + 0.015 ));
...
origami.redraw = function(){
	...
	for(var i = 0; i < 6; i++){
		var dir = (i%2 * 2)-1;
		this.cp.crease(new Line(this.touchPoints[i].position.x, this.touchPoints[i].position.y, 1, dir*.04));
	}

	for(var i = 0; i < this.xs.length-1; i++){
		this.cp.creaseAndReflect( new Line(this.xs[i], this.bounds.size.height*0.5, 0, 1) );
	}

	this.cp.clean();

	this.cp.creaseAndReflect( new Ray(0, this.touchPoints[6].position.y, 1, -0.5) );
	this.cp.creaseAndReflect( new Ray(0, this.touchPoints[7].position.y, 1, 0.5) );
	this.cp.clean();
