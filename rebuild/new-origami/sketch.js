...
paper.draw();
folded.draw();
folded.markLayer = RabbitEar.svg.group();
...
folded.onMouseDown = function(event){
	foldPoint = undefined;
	folded.startPoint = [event.x, event.y];
	folded.endPoint = [event.x, event.y];
}
...
function update(){
	if(foldLine){
		...
		let linePoint = foldLine.point;
		let lineVector = [foldLine.direction.x, foldLine.direction.y];
		var result = RabbitEar.fold.valleyfold(foldedFoldFile, linePoint, lineVector, foldPoint);
...
function updateCreaseLine(point1, point2){
	RabbitEar.svg.removeChildren(folded.markLayer);
	var vec = {x:point2[0] - point1[0], y:point2[1] - point1[1]};
	var d = Math.sqrt(Math.pow(vec.x,2) + Math.pow(vec.y,2));
	if(d <= 0){ d = 0.000001; }
	var normalized = {x:vec.x / d, y:vec.y / d};

	var svgLine = RabbitEar.svg.line(
		point1[0] + normalized.x * 10,
		point1[1] + normalized.y * 10,
		point1[0] - normalized.x * 10,
		point1[1] - normalized.y * 10,
		'valley');
	folded.markLayer.appendChild(svgLine);
	var node0 = RabbitEar.svg.circle(folded.startPoint[0], folded.startPoint[1], 0.01, 'blue-node');
	var node1 = RabbitEar.svg.circle(folded.endPoint[0], folded.endPoint[1], 0.01, 'blue-node');
