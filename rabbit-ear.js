...
(function (global, factory) {
  ...
  const make_face_walk_tree = ({ faces_vertices, faces_faces }, root_face = 0) => {
    if (!faces_faces) {
  ...
  const CreasePattern = function () {
    return Object.assign(
      Object.create(prototype$1),
      ...Array.from(arguments).filter(a => fold_object_certainty(a)),
      { file_spec, file_creator }
    );
  };
  Static$1(CreasePattern);
  CreasePattern.prototype = prototype$1;
  ...
  const clip_line_in_boundary = ({ vertices_coords, vertices_edges, edges_vertices, edges_assignment, boundaries_vertices }, lineVector, linePoint) => {
    ...
    return polygon.clipLine(math.line(lineVector, linePoint));
  };
  ...
  const split_convex_polygon$1 = function (
    ...
    if (edges_intersections.length === 2) {
      new_v_indices = edges_intersections.map((el, i, arr) => {
        const diff = add_vertex_on_edge_and_rebuild(graph, el.point, el.i_edges);
        arr.slice(i + 1)
    ...
    const faces_map = remove_geometry_indices(graph, "faces", [faceIndex]);
    return {
      faces: {
        map: faces_map,
        replace: [{
  ...
  const prepare_to_fold = function (graph, point, vector, face_index) {
    ...
    graph["faces_re:coloring"] = make_faces_coloring_from_matrix(
  ...
  const fold_through = function (
    ...
    folded["faces_re:coloring"] = make_faces_coloring_from_matrix(
      folded_faces_matrix
    );
  ...
  const core = Object.assign(Object.create(null), {
    ...
    add_edges,
    split_face: split_convex_polygon$1,
    fold_through,
  },
    keys$1,
    make,
